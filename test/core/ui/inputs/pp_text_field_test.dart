...
AnimatedContainer _box(WidgetTester tester) =>
    tester.widget<AnimatedContainer>(find.byKey(const Key('pp_field_box')));
...
void main() {
  ...
  testWidgets(
    'errorText is appended to the semantic label so it is announced',
    (tester) async {
      await tester.pumpWidget(
        wrap(
          const PPTextField(
  ...
  testWidgets('external focus node is not disposed by the field', (
    ...
    await tester.pumpWidget(
      MaterialApp(
        theme: ppDarkTheme(),
        home: Scaffold(
          body: PPTextField(label: 'Email', focusNode: node),
        ),
      ),
    );
  ...
  testWidgets('passes textInputAction and onSubmitted through', (tester) async {
    String? submitted;
    await tester.pumpWidget(
      MaterialApp(
        theme: ppDarkTheme(),
  ...
  testWidgets('tabularFigures opts the field into lining tabular digits', (
    tester,
  ) async {
    // The body scale is proportional by default, so a numeric field
    // (birth year, max HR) has to ask. Both the entered text and the
    // hint take the feature, or the hint shifts when it is replaced.
    await tester.pumpWidget(
      MaterialApp(
        theme: ppDarkTheme(),
        home: const Scaffold(
    ...
    final field = tester.widget<TextField>(find.byType(TextField));
    expect(field.style!.fontFeatures, ppTabularFigures);
    expect(field.decoration!.hintStyle!.fontFeatures, ppTabularFigures);
  });

  testWidgets('non-numeric fields stay proportional by default', (
    tester,
  ) async {
    await tester.pumpWidget(
      MaterialApp(
        theme: ppDarkTheme(),
        home: const Scaffold(body: PPTextField(label: 'Name')),
      ),
    );
    final field = tester.widget<TextField>(find.byType(TextField));
    expect(field.style!.fontFeatures, isNull);
  });
}
